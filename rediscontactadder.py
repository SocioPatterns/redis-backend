...
class RedisContactAdder:
    ...
    def __init__(self, run_name="", start_time='', deltat=20, host='localhost', port=6379, password=None):
        self.RUN = run_name
        self.DELTAT = deltat
    ...
    def __add_interaction(self, actor_id1, actor_id2):
        #if self.rdb.setnx('%s:interaction:%d_%d' % (self.RUN, actor_id1, actor_id2), 'interaction', (actor_id1, actor_id2)):
        #    self.rdb.incr('%s:interactions' % self.RUN) # number of interactions found in the caption
        interaction_key = '%s:interaction:%d-%d' % (self.RUN, actor_id1, actor_id2)
        self.if_new_interaction_incr_N(keys=[interaction_key, str(self.run_key)], args=[actor_id1, actor_id2], client=self.pipe)
    ...
    def __add_actor(self, actor_id):
        ...
        self.if_new_actor_incr_N(keys=[actor_key, str(self.run_key)], args=[actor_id], client=self.pipe)
        self.pipe.sadd(self.actors_key, actor_key)
        return actor_key
        #if self.rdb.hsetnx('%s:actor:%d' % (self.RUN, actor_id), 'name', actor_id):
        #    self.rdb.incr('%s:actors' % self.RUN) # number of actors met in the caption
        #self.rdb.sadd('%s:frame:%d:actors' % (self.RUN, self.last_frametime))
    ...
    def __interaction_frame(self, interaction_key, frame_time):
        ...
        #self.pipe.sadd('%s:interactions' % self.current_frame_key, interaction_key)
        #self.pipe.hincrby('%s:interaction_freq' % self.current_frame_key, interaction_key)
